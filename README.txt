...
O servidor cria um socket TCP (socket.AF_INET, socket.SOCK_STREAM).
Ele associa o socket ao endereço localhost na porta 12345.
O servidor entra em modo de escuta com listen() para aceitar conexões.
Quando um cliente se conecta, o servidor aceita a conexão com accept().
O servidor lê a mensagem do cliente usando recv() e imprime no terminal.
Ele então responde com "Hello World!" usando sendall().
A conexão é fechada após o envio da mensagem.
Cliente (tcp_client.py)
O cliente cria um socket TCP.
Ele se conecta ao servidor em localhost na porta 12345 usando connect().
O cliente envia uma mensagem para o servidor com sendall().
Ele recebe a resposta do servidor e imprime a mensagem "Hello World!".
O cliente fecha o socket após a troca de mensagens.
